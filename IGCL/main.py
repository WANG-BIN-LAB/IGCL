...
from recognition import custom_recognition_by_correlation
                ...
                if folder1 != folder2:  # Ensure the two folders are different
                    ...
                    for epoch in range(1, epochs + 1):
                        ...
                        if epoch % 5 == 0:
                            ...
                            emb_train = emb_train.reshape(100, 360, 360)
                            emb_test = emb_test.reshape(100, 360, 360)

                            y_pred = custom_recognition_by_correlation(emb_train, emb_test, y_tes)

                            # Calculate accuracy
                            acc_2 = accuracy_score(y_tra, y_pred)

                            y_pred = custom_recognition_by_correlation(emb_test, emb_train, y_tra)

                            # Calculate accuracy
                            acc_test_1 = accuracy_score(y_tes, y_pred)

                            print(f'Evaluation {folder1}->{folder2} accuracy: {acc_2:.4f}')

                            print(f'Evaluation {folder2}->{folder1} accuracy: {acc_test_1:.4f}')

                            acc_all = acc_2 + acc_test_1
                    ...
                    # Regional identification
                    emb_train = emb_train.reshape(469, 360, 360)
                    emb_test = emb_test.reshape(469, 360, 360)
                    ...
                    print(f'Testing {folder1}->{folder2} accuracy: {acc_2:.4f}')
                    ...
                    print(f'Testing {folder2}->{folder1} accuracy: {acc_test_1:.4f}')
                    ...
                    # Write information to a file
                    with open('.txt', 'a') as file:
                        file.write(f'Testing {folder1}->{folder2} accuracy: {acc_test_1:.4f}\n')